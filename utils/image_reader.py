...
def random_crop_and_pad_image_and_labels(image, label, crop_h, crop_w, ignore_label=255, batch_size=None):
    if batch_size:
        ...
        combined_crop = tf.random_crop(combined_pad, [batch_size,crop_h,crop_w,4])
        ...
        img_crop.set_shape((batch_size, crop_h, crop_w, 3))
        label_crop.set_shape((batch_size, crop_h,crop_w, 1))
    else:
        label = tf.cast(label, dtype=tf.float32)
        label = label - ignore_label # Needs to be subtracted and later added due to 0 padding.
        combined = tf.concat(axis=-1, values=[image, label])
...
def read_images_from_disk(input_queue, input_size, random_scale, random_mirror, ignore_label, img_mean): # optional pre-processing arguments
    ...
    img_r, img_g, img_b = tf.split(axis=-1, num_or_size_splits=3, value=img)
